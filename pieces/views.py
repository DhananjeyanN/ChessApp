...
def join_game(request):
    ...
    if not in_game:
        ...
        return JsonResponse({'status':'prev_game_in_progress', 'gameplay_id':prev_game_id}, status=200)
...
def close_game(request):
    game = GamePlay.objects.filter(white_player=Player.objects.get(user=request.user), is_ready=True)[0]
    game.completed = True
    game.save()
