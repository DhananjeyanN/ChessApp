...
class Square:
    ...
    def serialize(self):
        return json.dumps({
            'piece': self.__piece.serialize() if self.__piece is not None else None,
            'color': self.color
        })
...
class BoardLog:
    ...
    def serialize(self):
        data = {
            'logs': [[log[0].serialize(), log[1], log[2]] for log in self.logs],
            'captured_white': [piece.serialize() for piece in self.captured_white],
            'captured_black': [piece.serialize() for piece in self.captured_black],
            'promoted': [[promo[0].serialize(), promo[1].serialize(), promo[2]] for promo in self.promoted]
        }
        return data
    ...
    def deserialize(data):
        ...
        captured_black = data['captured_black']
        promoted = data['promoted']
        boardlog = BoardLog()
        boardlog.logs = [[Piece.deserialize(log_entry[0]), tuple(log_entry[1]), tuple(log_entry[2])]for log_entry in logs]
        boardlog.captured_white = [Piece.deserialize(piece) for piece in captured_white]
        boardlog.captured_black = [Piece.deserialize(piece) for piece in captured_black]
        boardlog.promoted = [[Piece.deserialize(log_entry[0]), Piece.deserialize(log_entry[1]), tuple(log_entry[2])] for log_entry in promoted]
        return boardlog
...
class Piece:
    ...
    def deserialize(data):
        ...
        if piece_class and issubclass(piece_class, Piece):
            ...
            raise ValueError(f'Invalid piece type: {data["type"]}')

    def __str__(self) -> str:
        return f"Piece(color={self.__color})"
...
class Board:
    ...
    def serialize(self):
        data = json.dumps([[self.board[x][y].serialize() for y in range(8)] for x in range(8)])
        return data
    ...
    def deserialize(data):
        ...
        for x in range(8):
            for y in range(8):
                board.board[x][y] = Square.deserialize(data[x][y])
        return board
...
class Game:
    ...
    def deserialize(data):
        ...
        log = BoardLog.deserialize(data['log'])
        turn = data['turn']
